...
Generalised supporting, contradicting, and qualifying observations from these engagements are presented under [Formative Insights from Exploratory Engagements](#sec:formative-insights) in the Discussion.
...
Where the AAC captures planning intent, Policy Cards enforce runtime constraints; together they span the full lifecycle from design through deployment to audit (see Discussion).
...
Selected completed AAC use cases are being followed prospectively, with expectations recorded before implementation so that expected and realised outcomes can later be compared.
