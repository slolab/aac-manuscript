...
Per-task feasibility can link to Model Cards [@doi:10.1145/3287560.3287596] via identifier fields where a trained model is in use.
Per-task feasibility also includes structured **risk assessments** that parallel benefit metrics: each risk specifies a category (technical, data, compliance, operational, ethical, or adoption), title, description, likelihood and impact ratings (low through critical), a mitigation strategy, and a status (identified, mitigated, accepted, or resolved).
This risk model captures the intuitive counterpart to benefits---what could go wrong, how likely and severe, and how to address it---enabling balanced planning conversations.
...
### Benefit Quantification Model and Risk Assessment
...
Per-task risk assessments use six categories that parallel and complement the benefit dimensions.
...
The version of the schema and application at the time of this publication are 0.13.1.
