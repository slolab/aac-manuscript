...
**P-Plan** [@pplan]: User expectations and requirements are structured using the Plan Ontology, with each requirement represented as a `p-plan:Step` within a `p-plan:Plan`, and dependencies modeled as `p-plan:isPreceededBy` relationships.
...
Together, these standards deliver the four FAIR facets at the level of the exported canvas:

- **Findable**: the schema profile resolves at a persistent [w3id.org](https://w3id.org/aac/) identifier, and canvas entities (projects, datasets, persons) carry persistent identifiers where available.
- **Accessible**: the self-contained RO-Crate bundles a human-readable HTML preview (`ro-crate-preview.html`) alongside the machine-readable JSON-LD (`ro-crate-metadata.json`), retrievable without the running application.
...
- **Reusable**: controlled vocabularies, PROV-O and P-Plan provenance, and licence and `DUO` terms document the conditions and context for reuse.
